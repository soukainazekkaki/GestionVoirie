...
<div id="band" class="container text-center">
  <h3><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Modifier voirie </h3>
  <br>
  <div class="row">
    

  <!-- multistep form -->
<form id="msform" action="{{ route('responsable.updatevoirie',$advoiries->id) }}" method="post">

@csrf

<!-- on utilise tjrs cette methode quand on veut modifier les données -->
<!-- cette methode nous permet de faire le mise à jour des données du voirie -->
 @method ('PUT') 
<!-- et pour supprimer on utilise la method('DELETE')-->
  ...
  <option selected>{{$advoiries->nomcommune}}</option>
  ...
  <option value="Ketama">Ketama</option>
    ...
    <input type="text" name="titre" value="{{$advoiries->titre}}"  />
    <div>
    ...
    <textarea class="mdl-textfield__input" name="description" rows="4" id="description" >"{{$advoiries->description}}"</textarea>
    <div>
    ...
    <input type="text" name="nomresponsable" value="{{$advoiries->nomresponsable}}" />
    <div>
  ...
  <option selected>"{{$advoiries->ville}}"</option>
...
<div class="file-field">
  ...
  <div>
  <input type="text" name="x" value="{{$advoiries->x}}" />
  <div>
    <span class="text-danger">@error('x'){{ $message }}@enderror</span><div>

  <input type="text" name="y" value="{{$advoiries->y}}" />
  <div>
    ...
    <input type="submit" name="submit" data-toggle="modal" data-target="#myModal" class="submit action-button" value="Modifier" />
  </fieldset>
